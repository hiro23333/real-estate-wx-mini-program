...
import { getBannerList, getPropertyList, getTagList, getCommunityList } from '../../api/property';
import { Banner, Property, Tag, Community } from '../../types/models';
...
// --- 定义带有签名URL的类型 ---
interface BannerWithSignedUrl extends Banner {
    signedUrl?: string;
}
...
interface IndexPageMethods {
    ...
    onLoadMore(): void;
    onBannerTap(e: WechatMiniprogram.TouchEvent): void;
...
Page<IndexPageData, IndexPageMethods>({
            ...
            if (res.code === 0 && res.data) {
                const bannersWithSignedUrls = await Promise.all(
                    res.data.map(async (banner) => ({
                        ...banner,
                        signedUrl: await getOssSignedUrl(banner.oss_path)
                    }))
                );
                this.setData({ banners: bannersWithSignedUrls });
            }
        } catch (error) {
    ...
    onBannerTap(e: WechatMiniprogram.TouchEvent) {
        ...
        if (propertyId) {
            wx.navigateTo({ url: `/pages/property-detail/property-detail?id=${propertyId}` });
        }
    },
    ...
    onLandlordTap() {
        if(!wx.getStorageSync('userInfo')){
            ...
            wx.navigateTo({ url: '/pages/landlord-form/landlord-form' });
        }
    },
});